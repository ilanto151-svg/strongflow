...
function parseEx(r) {
  return {
    ...
    distance: r.distance || '',
    speed: r.speed || '',
    aerobic_equipment: r.aerobic_equipment || null,
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO exercises (
        ...
        rpe, img_data, img_url, link, intervals, set_overrides, sort_order, distance, speed, aerobic_equipment
      ) VALUES (
        $1, $2, $3, $4, $5, $6, $7, $8,
      ...
      [
        ...
        ex.rest || '',
        ex.notes || '',
        (ex.rpe != null && ex.rpe !== '') ? Number(ex.rpe) : 5,
        ex.img_data || null,
        ex.img_url || null,
        ex.link || null,
        ex.intervals || '[]',
    ...
    await pool.query(
      `UPDATE exercises
       SET
         ...
         distance = $18,
         speed = $19,
         aerobic_equipment = $20
       WHERE instance_id = $21 AND patient_id = $22`,
      [
        ex.name,
        ex.image || '',
        ex.description || '',
